...
class VoiceGame(cocos.layer.ColorLayer):
    ...
    def __init__(self):
        ...
        self.logo = cocos.sprite.Sprite('baba.png')
    ...
    def collide(self):
        px = self.sm.x - self.floor.x
        for b in self.floor.get_children():
            if b.x <= px + self.sm.width * 0.8 and px + self.sm.width * 0.2 <= b.x + b.width:
                if self.sm.y < b.height:
                    self.sm.land(b.height)
                    break
...
cocos.director.director.init(caption="Let's Go! Sun Ming!")
